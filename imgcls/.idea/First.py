...
for subdir, dirs, files in os.walk(datadir):
    for file in files:
        # paths to cats and dogs dir should be the same name as the foldername.
        path = os.path.join(subdir, file)
    for img in os.listdir(path):
        # to convert the image to greyscale
        img_array = cv2.imread(os.path.join(path, img), cv2.IMREAD_GRAYSCALE)
        # to take the color image comes as blue green red in cv2
...
# length*width of the resized image read by the amount of pixels.
...
# 1 here stands for the greyscale if this is color number = 3
